...
RSpec.describe Operations::CreateOrder, type: :model do
  describe '.execute' do
    ...
    context '既存の在庫数の範囲内で注文を受け付けた場合' do
      subject(:response) { Operations::CreateOrder.execute(params) }
      let(:params) do
        {
          user_id: user.id,
        ...
        expect { 
          ...
          threads << Thread.new do
            ActiveRecord::Base.connection_pool.with_connection do
              Operations::CreateOrder.execute(params1)
            end
          end
          threads << Thread.new do
            ActiveRecord::Base.connection_pool.with_connection do
              Operations::CreateOrder.execute(params2)
            end
          end
          threads.each(&:join)
         }.to change(ProductStock, :count).by(2)
      end
    end
